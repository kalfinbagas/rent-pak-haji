...
/// Adds structured logging (start time, duration, errors) around each consumer lifecycle call.
///
/// Inspired by SERA's WorkerService pattern — adapted for RabbitMQ consumers.
/// </summary>
public sealed class WorkerOrchestrator
{
    ...
    public async Task StartConsumerAsync(IConsumerService consumer, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogError(ex,
                "Consumer {Consumer} failed to start: {Message}", consumer.Name, ex.Message);
            throw;
        }
    }
    ...
    public async Task StopConsumerAsync(IConsumerService consumer, CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogWarning(ex,
                "Consumer {Consumer} did not stop cleanly: {Message}", consumer.Name, ex.Message);
        }
    }
}
